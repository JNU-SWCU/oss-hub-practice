...
4. **환경 변수 등록**:
   ...
   - `DATABASE_URL`은 반드시 pooler(포트 `6543`, `pgbouncer=true`) 형식이어야 하며, 그렇지 않으면 backend가 기동 시점에 명시적으로 거부합니다.
...
5. **CI 통과**: PR을 열면 GitHub Actions가 아래 7개 명령을 순서대로 실행합니다. 실패한 스텝은
   ...
   4. `pnpm workspace:lint`
   5. `pnpm workspace:typecheck`
   6. `pnpm --filter ./backend test`
   7. `pnpm workspace:build`
